...
struct SourceLookupReport final {
    ...
    [[nodiscard]] bool found() const noexcept {
        return key_valid && source_available && !matches.empty();
    }

    [[nodiscard]] bool unique() const noexcept {
        return key_valid && source_available && matches.size() == 1U;
    }
...
public:
    ...
    // provider_key must already be canonical under normalization_flags.
    // Ambiguous normalized identities are returned together; the registry does
    // not invent an original-game winner policy for comparator-equal keys.
