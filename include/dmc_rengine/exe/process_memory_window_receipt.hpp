...
// Deterministic v2 receipt for one bounded live main-module range. Raw bytes are
// omitted by default. The process creation FILETIME is mandatory so later R2B
// and R3 evidence cannot be joined solely by a reusable PID/module-base pair. A
// mapping expectation is valid only when both the canonical source artifact SHA
// and canonical window SHA are present.
