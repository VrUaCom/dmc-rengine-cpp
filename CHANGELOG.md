...
- `outside_every_closure` is described as unreachable "under any assumption the file supports". **Too strong.** The closure follows calls and dispatch; the file records two more edge kinds. A **funclet** named in a C++ `FuncInfo` or an SEH scope table is entered by the runtime while its parent is on the stack; a function whose start is loaded by a **`lea r64,[rip+disp32]` inside reachable code** can be called by whatever receives it. Re-closing from the tool's 5,748 over its own call edges reproduces exactly 5,748; adding funclets absorbs **314**, taken addresses **48**, both **362** — leaving **1,279** over 557,004 bytes. By count −22%; by mass barely moved, because funclets are small. The population note's substance survives; the flag's guarantee should read "over calls and dispatch";
- **the exception metadata, parsed rather than bounded.** Every handler is one of four: `__GSHandlerCheck` (1,408, no funclets), `__CxxFrameHandler3` (518), the cookie-checking C++ variant at `0x3458E8` (279) and `__C_specific_handler` (5). All 797 `FuncInfo` pointers land on a valid magic — none misses. That yields 792 `FuncInfo`, 2,240 unwind states, 4,150 IP-map entries and **five try blocks with five catch clauses in the whole image**: the engine uses C++ EH almost only for destructor unwinding, which a port can treat as RAII;
- **a correction to the last note.** Its 332 unwind-data references were a lower bound, and its 526 "ordinary `.rdata`" references were called the likeliest home of a second mechanism. With full metadata extents most of those 526 were `FuncInfo`, unwind maps and IP maps. The population now partitions exactly: **794** called by another unreached function, **383** no-caller exception targets (327 unwind actions, 49 IP-map-only, 5 catch, 5 SEH), **108** named only as the parent of their own split fragment, **99** referenced by other data or code, **257** referenced nowhere outside `.pdata`;
- **the second mechanism is in code.** Verified instruction by instruction, reached code at `0x254BB3` passes constructor `0x254C40` and destructor `0x254D00` with element size `0xE00` and count `0x27` to `0x345B24`, and `0x254CCA` passes the destructor to `0x345B94` — MSVC's array constructor and destructor iterators, with 145 and 360 callers. The element destructor is only called by the helper through the pointer. Census with a control: 13,225 `lea` patterns hit **49** unreached starts (23 from reached code) and **0** of 1,641 matched non-starts. The previous scan's 260 raw-dword `.text` matches were coincidences — x64 code does not take a function address as an absolute dword;
- `research/exe/measure_closure_gaps.py` reproduces every re-closing figure, SHA-gated, identifying handlers by the import they jump through and `FuncInfo` by magic rather than by address; nine tests need no executable and **all eight mutations are killed**. Changing the tool's own flag is left for its own increment, since it moves a published counter.

#### What the 1,641 are: half of them are pointed at, and 332 are funclets

- the reachability note listed what the unreached population is *not* and left what it *is* unexamined. It also carries a wrong figure: **"nothing in the image refers to them at all — 847"** counts functions with **no call edge**, which is what the report's `callers` field measures. Scanning the whole file for each start as a 32-bit or 64-bit address, excluding `.pdata`'s own fields, finds **972 of the 1,641 referenced somewhere**; of the 847 with no caller, **597 are referenced** and only **250 are not**. The mystery was three times smaller than published;
- **the scan is measured against a control, not asserted.** The same scan over 1,641 aligned addresses from the same span that are *not* function starts returns **133 — 8.1%**, against **972 — 59.2%** for the real ones. Seven times the background. Subtracting a uniform background would put the genuine figure near 840, but the background is not uniform, so the honest statement is the pair of rates;
- **332 are referenced from exception-handling data** — catch and unwind funclets, entered by the runtime from handler data and never by a call. No call graph can record that route, not because this one is incomplete but because the mechanism is not a call. The earlier note's "every route into it is one this file does not record" is exactly right; this names one. The 332 is a **lower bound** (the unwind extent computed covers the fixed part and handler pointer, not the variable data after it). A further **526** are referenced from ordinary `.rdata` — tables that are neither vtables nor the ten retained function-pointer runs, and the next question;
- **it is not library residue.** 92% of the library tail at or above `0x33F000` is unreached (405 of 442) — but that tail is only **6.7% of the unreached bytes**. Ninety-three per cent of the unreached mass is game code. The largest run by count is the CRT stub region (360 functions, 47 bytes each); the largest by mass is `0x1FED40`–`0x2118F0`, 119 functions over 76,769 bytes;
- **and it is not compiler noise.** Against the 5,748 inside the closure, the unreached call an import **11.1% vs 6.1%** and reference a string **3.7% vs 1.4%** (12.5% and 4.7% for the game-code part), with a median size of 191 against 227. **77 install a vtable** — constructors for classes nothing in the recorded graph ever builds, the same wall the RTA-starves finding describes, from the other side. The 0% bound to a vtable slot reproduces exactly and is a consistency check, not a finding.

#### The report was not partial, and regenerating it takes one second

- yesterday's note said the committed function-map report "is a partial artefact of 2,889 functions ... so it cannot answer the question." **Both halves are wrong.** Its own summary says `functions: 7389` and its top level says `functions_emitted: 2889` — different numbers because `map-functions` emits per-function records only for the attributed subset unless given `--all`, while the walk and every counter cover all 7,389. The report states both figures and I read one of them;
- running the tool today gives `functions: 7389`, `walks_complete: 7381` — **identical** — with every shared counter matching exactly except six that *this session's own analysis* changed (`computed_index_references` 16→0, `constant_index_references` 186→189, `name_tables_referenced` 27→17, `name_tables_unreferenced` 218→51, `with_name_table` 45→40, `with_string_reference` 136→139). The real limitation was narrower: the committed report predates counters later work added, `outside_every_closure` (the 1,641 several notes cite) among them;
- **what matters more than the miscount is that I never checked.** Regeneration takes **one second**, and I wrote that an instrument could not answer a question without running it. Two days earlier I recorded a habit after nearly publishing that the CRC machinery was unreachable — before trusting a caller count of zero, read the bytes before the address. This is the same failure one level up, and unlike that one it was **published** rather than caught;
- **the committed report is now current**: same emission policy, 2,889 of 7,389, 2.08 MB → 2.76 MB. All **102 declared figures verify against it, 0 stale, 0 unknown**, suite green. The figure checker could never have caught this drift and that is by design — it regenerates and compares against the fresh report, which answers "have the numbers moved" and says nothing about "is the committed artefact current". Two questions, now both answered;
- **and the layer-B question is answered as a bracket.** The resource open is depth 11, three callers, reachable from an export; layer B's open depth 12, one caller, likewise. Layer B's read and seek are **not** outside every closure — reachable under the loosest sound assumption — and their calling functions have no direct callers, so whatever reaches them does so through dispatch. That is an over-approximation, not a proof. Against it, **`0x338140`, one of the resource open's own three callers, is itself outside every closure**: one of the three ways in is a way nothing takes;
- a convention that will trip the next comparison: the report's `callers` counts **distinct calling functions, not call sites**, which is why layer B's open shows one caller where a byte census found two sites, both inside the same continuation.

#### Why layer B exists: it is the back end of two open modes nothing selects

- the file-access note found two independent file layers over one platform and closed on "nothing here says why the engine has both"; the mount-registration note added a second separating axis and left it open. **`0x2FF12` answers it.** The resource open has three resolution paths: the prefixed mount search when mode bit 1 is clear, and with bit 1 set, two more chosen by bit 9 — a set bit derives the executable's directory and calls `0x49120`, a clear bit builds a descriptor and calls `0x49120` too. **`0x49120` is layer B's owner of `CreateFileA` and `GetFileSize`**, its only two callers in the image are those two sites, and its address appears as a qword nowhere in the file, so there is no indirect path in either;
- **and no caller selects either mode.** Both need bit 1 set, and all three direct callers load `edx = 1`, so **layer B's open is unreachable from the shipped call surface**. That also explains the shape the earlier note measured — 16 callers in against 10 callees out, two users per dependency where layer A had forty-six — because its entry point is the back end of a mode the game never asks for;
- **not the same as dead, and I did not pretend to settle it.** Layer B's read at `0x49200` has callers (`0x2F989`, and a thunk at `0x2FF70` reached from `0x33820E`, itself inside one of the resource open's three callers), and its seek at `0x49290` has two. Whether *those* callers are reachable is **not** established here, and the obvious instrument cannot settle it: the function-map report committed under `evidence/` is a partial artefact of 2,889 functions with its depth fields absent. No guess was substituted. What follows is narrower and still useful — the open being unreachable means any live use must get its handle by a route this pass did not find;
- **the two layers differ in interface style, not just in shape.** Both read and seek call sites load a **32-bit value from the descriptor at `+0x30`** into the first argument — not a pointer — while layer A's read, seek and size all take pointers. So layer B is an id-based API over a handle table and layer A is pointer-based, which explains how two layers coexist without either wrapping the other: their calling conventions are not compatible enough to wrap.

#### A hard cap of one hundred open resources, and two sections that were already closed

- I set out to check §1C and §1E of the 2026-09-05 reconciliation, having found §1A/§1B/§1D sound. **They hold, and they were never open**: `docs/gdspaces/l2-exe-reconciliation-2026-08-26.md` §1.1 already states that "inside `OpenGameResource` the raw instructions confirm" basename extraction, the `0x400` buffer, the six prefixes, phase 1 before phase 2, the resolver address and early failure on candidate construction, with §1.2/§1.3 adding the mask-bit gating and the `0x0E` normalisation — and marks the reverse CLOSED. My re-read agrees on every point and adds nothing to them. Detail I can put beside them: the **prefix is the inner** loop over six and the provider pass the outer over two, and the mask is literally `pass + 1` rather than two constants; a join failure exits **both** loops. §1E is visible as two different jump destinations in `0x327473` — a lookup miss to the next mount, a construction failure to the cleanup that frees and returns zero;
- **new: a hard cap of one hundred.** Before any search, the open calls `0x48B20`: enter a critical section at `0xC19AE0`, scan from `0xC18E78` in steps of `0x20` for the first record whose in-use byte is clear, **stop at index `0x64`**, mark it, zero `+0x08`/`+0x10`, set `+0x00` to `-1`, leave. So the open-resource table is **100 records of 0x20 bytes at `0xC18E60`**, and when all hundred are taken the allocator returns null and the request fails with `-1` — no wait, no growth, no eviction. The release at `0x48DF0` clears **only** the in-use byte; it does not close the stream at `+0x08`. On the one path that calls it no stream is assigned yet, so nothing leaks, but the release gives no guarantee;
- **new: where a resource's size comes from.** On success the open passes the stream to `0x3272A0` and stores the result at slot `+0x10`. That is the **third** stream dispatcher — the one a byte scan found after the mount-list note claimed there were two — and its archive branch is the two-byte accessor at `0x0629C0` returning the uncompressed size. The chain closes from both ends: the earlier note asked why a size accessor lives outside the file layer and answered with where it sits; this is the answer from the other side, which is who needs it;
- **new: two modes nothing selects.** The prefixed search runs only when **bit 1 of the mode is clear** — the gate is written as two tests but the second implies the first, so one of them can never decide anything. With bit 1 set, `0x2FE2A` tests **bit 9** and on a set bit derives the executable's own directory exactly as the bootstrap does, while a clear bit 9 goes to `0x2FF12`. All three direct callers load `edx = 1`, so **bits 1 and 9 are never set anywhere in the image** and both alternative paths are unreachable from the shipped call surface.

#### Three functions of zlib, a dead deflate half, and the archive teardown

- **the engine's entire zlib surface is three functions.** A census of every direct call or jump in `.text` crossing into `0x340000`–`0x344000` from outside finds `inflate` (`0x341900`, from the two drivers), `inflateEnd` (`0x343110`, from the entry destructor and the seek) and `inflateInit2_` (`0x343170`, from the arming). Forty-one further targets in the range are called only from within it, and the only qwords in the image pointing into it are ten entries of one table. That states a port's replacement surface exactly;
- **the deflate half is linked and dead.** `deflate.c`'s configuration table sits at `.rdata 0x50AAA8` — ten sixteen-byte entries whose function pointers fall `0x340320` once, `0x3405E0` three times, `0x340A50` six times, which is that library's exact one/three/six split — with the `"deflate 1.2.5"` copyright at `0x50AA51`. Nothing calls it: the table is reachable only from the compressor entry, which is not among the three edges, and there is no indirect path in. **A port reproduces no compression on this path**;
- **yesterday's CRC statement, sharpened.** The check routine is live with 13 call sites all inside the library. The stream is armed with window bits `-15`, and the library computes a check value only while parsing a wrapper — so those sites lie on wrapper handling this mode never enters: live in the module, unreachable in this engine's use. That last step is an **inference** from the mode constant and the library's structure, not a trace of each site, and is flagged as such; the read claim is unchanged — no call site is in the archive path and the stored CRC at entry `+0x18` is never loaded;
- **`0x328210` closes the archive object's lifecycle.** Walk the list from `+0x28` through `+0x48` freeing three strings and the node; free the sorted array at `+0x30` and null it; tail-jump to `free` for the comment at `+0x20`. Same shape as the inline cleanup at `0x328B60`, written twice. It nulls **only** `+0x30`, leaving the head, count and comment pointer stale, so **it is not idempotent** — a second call walks a freed list. Its one caller frees the containing mount immediately after, so nothing calls it twice;
- **a false edge, and the habit that catches it.** The first census run showed a fourth inbound edge, `0x24479A → 0x3430FF`. Neither address is code: `0x3430F0`–`0x343107` is a switch jump table of six RVAs into `inflate`, which the scanner disassembled as instructions. Yesterday's habit was to read the bytes *before* an address before trusting a zero count; its counterpart earned itself today — before trusting a surprising edge, read the bytes *at both ends*. This one would have put a call from the middle of the image into the middle of a library function.
