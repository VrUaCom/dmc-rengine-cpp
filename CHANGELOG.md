...
- **155 indexed arrays recovered**, with their element size and the field offsets the code reads inside them. On a held base the displacement is a field offset, so it must land inside the element — a free consistency check that splits 554 reads into 267 consistent array walks, 255 inlined character scans (a negative offset), and **32 at or past the element**, which is the 5.8% error bar on the element-size inference rather than something to hide;
- 28 of those arrays are read at more than one offset and so carry a partial element layout: `0x5D08A0` is a 20-byte element read at +4 +8 +12 +16, `0x5DE5B0` a 24-byte element at +0 +8 +16, `0x570600` a 12-byte element at +0 +4 +8. Layout only — where an array is, how wide its element is, which offsets are read — never contents;
