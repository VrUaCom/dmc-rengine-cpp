...
`RuntimeArchiveVolume::resolution_rank` is explicit: rank `0` is consulted first among archive mounts. Canonical filename validation inside `valid() noexcept` is allocation-free and rejects alternate spellings such as leading-zero indices.
