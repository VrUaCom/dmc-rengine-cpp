...
#include "../src/cli/scm_normal_authoring_commands.hpp"
...
std::vector<std::byte> fixture() {
    ...
    const auto normals = static_cast<std::size_t>(mesh_layout.normals_offset);
    for (std::size_t index = 0U; index < 3U; ++index) {
        put<float>(bytes, normals + index * 12U + 0U, 0.0F);
        put<float>(bytes, normals + index * 12U + 4U, 0.0F);
        put<float>(bytes, normals + index * 12U + 8U, 1.0F);
    }
...
int run_normal_cli(
    ...
    return dmc::rengine::cli::try_run_scm_normal_authoring_command(
...
int main() {
    ...
    assert(parsed.ok());
    const auto source_normal = parsed.document.objects[0].meshes[0].normals[0];
    assert(source_normal.x == 0.0F);
    assert(source_normal.y == 0.0F);
    assert(source_normal.z == 1.0F);

    auto authored_document = parsed.document;
    const Vec3f authored_normal{0.0F, 1.0F, 0.0F};
    const auto edit = set_vertex_normal(
        authored_document, 0U, 0U, 0U, authored_normal);
    assert(edit.ok());
    assert(edit.changed);
    ...
    const auto normal_offset = static_cast<std::size_t>(
        parsed.document.objects[0].meshes[0].normals_offset);
    auto expected = source;
    put_f32_le(expected, normal_offset + 0U, 0.0F);
    put_f32_le(expected, normal_offset + 4U, 1.0F);
    put_f32_le(expected, normal_offset + 8U, 0.0F);
    ...
    for (std::size_t index = 0U; index < source.size(); ++index) {
        ...
        changed = true;
        assert(index >= normal_offset && index < normal_offset + 12U);
    }
    assert(changed);
    ...
    const auto reparsed_normal =
        authored_parse.document.objects[0].meshes[0].normals[0];
    assert(reparsed_normal.x == 0.0F);
    assert(reparsed_normal.y == 1.0F);
    assert(reparsed_normal.z == 0.0F);
    assert(authored_parse.document.objects[0].bounding_radius == 2.0F);

    auto inverse_document = authored_parse.document;
    const auto inverse_edit = set_vertex_normal(
        inverse_document, 0U, 0U, 0U, source_normal);
    ...
    const auto invalid_edit = set_vertex_normal(
        invalid_document, 0U, 0U, 0U, Vec3f{nan, 0.0F, 1.0F});
    assert(!invalid_edit.ok());
    assert(!invalid_edit.changed);

    const auto out_of_range = set_vertex_normal(
        invalid_document,
        0U,
        0U,
    ...
    const auto test_root =
        ...
        "dmc-rengine-scm-normal-authoring-tests";
    ...
    assert(run_normal_cli(valid_input, valid_output) == 0);
    ...
    assert(run_normal_cli(valid_input, valid_output) == 15);
    cli_output.clear();
    assert(dmc::rengine::cli::scm_authoring_detail::read_file(
        valid_output, cli_output));
