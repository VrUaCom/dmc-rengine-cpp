...
std::vector<std::byte> wrapped_texture_fixture() {
    ...
    constexpr std::uint32_t mip_count = 7U;
    constexpr std::uint32_t payload_size = 2744U;

    const std::vector<std::byte> payload(payload_size, std::byte{0});
    const auto built = dmc::rengine::profiles::dmc3::Dmc3DdsProfile::build(
        width, height, dmc::rengine::profiles::dmc3::Dmc3DdsCompression::dxt1, payload);
    assert(built.ok());
    const auto& dds = built.bytes;
