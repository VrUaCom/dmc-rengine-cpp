...
int main() {
    ...
    // Preserve observed case and basename bytes. Normalization belongs to the
    // selected provider/backend, not this OpenGameResource candidate stage.
    ...
    // Long basename that fits the empty prefix but not the longest prefix is
    // rejected as a whole plan because the executable attempts all prefixes
    // through the same fixed-size candidate buffer.
