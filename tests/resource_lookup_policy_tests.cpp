...
int main() {
    ...
    const std::string embedded_nul{"room/st001\0evil.pac", 20U};
    ...
    // A manually altered request cannot claim a valid plan merely because the
    // already-built attempts remain internally self-consistent.
    ...
    // representation. The 0x400 destination bound is recovered; exact original
    // continuation after one oversized prefix remains unclaimed.
