...
int main() {
    ...
    // Child input order is not structural slot authority.
    ...
    // Every populated slot requires exact intrinsic bytes, even if unchanged.
    ...
    // PNST consumes the same recovered physical runtime-synth envelope while
    // remaining a distinct parser/format authority.
    ...
    // A valid zero-slot source can be rebuilt to the recovered 64-byte runtime
    // image without fabricating children or slot occupancy.
    auto zero = payload(
