...
void cleanup_repack_paths(const std::filesystem::path& path) {
    std::error_code error;
    std::filesystem::remove(path, error);
    std::filesystem::remove_all(
        std::filesystem::path{
            path.string() + ".dmc-rengine-repack.staging"},
        error);
}
...
int main() {
    ...
    // Layer 1 synthetic composition receipt only. This proves canonical
    // production components compose across two nested relative-slot levels and
    // an outer NBZ repack. It does not prove retail child identity or original
    // DMC3 consumption.
    ...
    cleanup_repack_paths(output_path);
    ...
    // Final synthetic receipt: recursive materialization reproduces the exact
    // requested leaf edit while nested and NBZ-level siblings remain exact.
