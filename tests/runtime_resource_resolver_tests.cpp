...
int main() {
    ...
    // Prefix order is the outer authority. A lower-precedence volume hit under
    // prefix 0 beats a higher-precedence volume hit under prefix 1.
    {
    ...
    // Within one candidate, highest contiguous archive volume wins.
    {
        SourceRegistry registry;
        mount(registry, "physical");
        mount(registry, "archive-2", {
        ...
        assert(report.probes[0].lookup.provider_key_valid);
        assert(report.probes[0].lookup.source_available);
    }

    // All archive candidates complete before physical attempt 6.
    {
        SourceRegistry registry;
        mount(registry, "physical", {
            make_resource(
    ...
    // Ambiguity in the highest-precedence mount stops immediately. No lower
    // volume or enumeration-order winner is invented.
    {
        SourceRegistry registry;
        mount(registry, "physical");
        mount(registry, "archive-2", {
    ...
    // Complete miss records 6 archive candidates x 3 archive mounts + 6
    // physical candidates.
    {
        SourceRegistry registry;
        mount(registry, "physical");
    ...
    // Missing runtime-bound source is configuration failure, not a resource miss.
    {
        SourceRegistry registry;
        mount(registry, "physical");
        mount(registry, "archive-1");
    ...
    // Invalid requests, including embedded NUL, fail before source probes.
    {
        SourceRegistry registry;
        mount(registry, "physical");
        mount(registry, "archive-2");
        ...
        const auto trailing = RuntimeResourceResolver::resolve(
            "room/", bootstrap, bindings, registry);
        assert(trailing.status == RuntimeResolutionStatus::invalid_request);
        assert(trailing.probes.empty());

        const std::string embedded_nul{"room/st001\0evil.pac", 19U};
        const auto nul = RuntimeResourceResolver::resolve(
            embedded_nul, bootstrap, bindings, registry);
        assert(nul.status == RuntimeResolutionStatus::invalid_request);
        assert(nul.probes.empty());
    }

    // Zero archives still leaves the archive pass conceptually before physical,
    // but with no archive source probes the first recorded probe is attempt 6.
    {
