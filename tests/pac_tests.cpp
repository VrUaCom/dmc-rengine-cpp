...
std::vector<std::byte> make_pac() {
    // 5 slots. slot 1 is empty. slot 3 aliases slot 0's offset.
    // Extents are inferred from the next distinct populated offset rather than
    // from slot ordering.
    ...
    bytes[0x20U] = std::byte{0xAA};
    bytes[0x30U] = std::byte{0xBB};
    bytes[0x40U] = std::byte{0xCC};
    return bytes;
}
...
int main() {
    ...
    // A zero-slot PAC is structurally representable and does not invent data.
