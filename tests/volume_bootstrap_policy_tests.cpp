...
int main() {
    ...
    assert(empty.present_after_first_gap.empty());
    ...
    assert(contiguous.physical_root_registered_before_archives);
    assert(contiguous.mount_list_is_prepend);
    assert(contiguous.first_missing_index == 3U);
    assert(contiguous.registered_archives.size() == 3U);
    assert(contiguous.registered_archives[0].index == 0U);
    assert(contiguous.registered_archives[0].filename == "DMC3-0.nbz");
    assert(contiguous.registered_archives[0].registration_order == 0U);
    assert(contiguous.registered_archives[0].resolution_priority == 2U);
    assert(contiguous.registered_archives[1].index == 1U);
    assert(contiguous.registered_archives[1].filename == "DMC3-1.nbz");
    assert(contiguous.registered_archives[1].registration_order == 1U);
    assert(contiguous.registered_archives[1].resolution_priority == 1U);
    assert(contiguous.registered_archives[2].index == 2U);
    assert(contiguous.registered_archives[2].filename == "DMC3-2.nbz");
    assert(contiguous.registered_archives[2].registration_order == 2U);
    assert(contiguous.registered_archives[2].resolution_priority == 0U);
    ...
    auto forged_filename = contiguous;
    forged_filename.registered_archives[1].filename = "DMC3-99.nbz";
    assert(!forged_filename.valid());

    auto forged_resolution = contiguous;
    forged_resolution.archive_resolution_order = {0U, 1U, 2U};
    assert(!forged_resolution.valid());
    ...
    assert(gap.registered_archives[0].index == 0U);
    ...
    auto forged_post_gap = gap;
    forged_post_gap.present_after_first_gap = {3U, 2U, 7U};
    assert(!forged_post_gap.valid());
    forged_post_gap.present_after_first_gap = {2U, 2U, 7U};
    assert(!forged_post_gap.valid());
    forged_post_gap.present_after_first_gap = {1U, 7U};
    assert(!forged_post_gap.valid());
    ...
    assert(missing_zero.archive_resolution_order.empty());
    ...
    assert(later_gap.registered_archives.size() == 2U);
