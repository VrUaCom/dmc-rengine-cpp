...
[[nodiscard]] std::vector<std::byte> texture_bundle() {
...
[[nodiscard]] std::vector<std::byte> st001_name_list() {
    ...
    const auto copy_at = [&](std::size_t offset, std::string_view value) {
        for (std::size_t index = 0U; index < value.size(); ++index) {
            bytes[offset + index] = static_cast<std::byte>(
                static_cast<unsigned char>(value[index]));
        }
    };
    copy_at(0U, "st001.ptx");
    copy_at(11U, "st001.scm");
    copy_at(22U, "st001.sch");
    return bytes;
}
...
[[nodiscard]] dmc::rengine::gdspaces::ContainerExpansion st001_expansion() {
    ...
    expansion.children.push_back(make_child(parent, 1U, true, texture_bundle()));
...
[[nodiscard]] dmc::rengine::gdspaces::ContainerExpansion em035_037_sparse() {
    ...
    for (std::uint32_t slot = 0U; slot < 95U; ++slot) {
        ...
        expansion.children.push_back(make_child(
            ...
            slot,
            populated,
            populated ? text_bytes("payload") : std::vector<std::byte>{}));
    }
    return expansion;
}
...
int main() {
    ...
    const auto index = index_payload(
        "st001.index",
        "st001_000.ukn\r\n"
        "st001_001 folder\r\n"
        "st001_002.scm\r\n"
    ...
    for (std::size_t index_value = 0U; index_value < expansion.children.size(); ++index_value) {
        assert(expansion.children[index_value].payload.resource.id == original_ids[index_value]);
    }
    ...
    const auto legacy2 = dmc3::LegacyExtractionNamingPlanner::build(slot2);
    assert(legacy2.valid());
    assert(legacy2.representation == dmc3::LegacyExtractionRepresentation::file);
    assert(legacy2.extraction_name == "st001_002.scm");
    ...
    const auto legacy3 = dmc3::LegacyExtractionNamingPlanner::build(slot3);
    assert(legacy3.valid());
    assert(legacy3.extraction_name == "st001_003.ukn");
    assert(legacy3.embedded_semantic_alias == "st001.sch");
    assert(legacy3.canonical_display_name == "st001_003.hits");

    // Real sparse-PAC topology recovered from em035_037: .index ordinals
    // 0..7 name physical populated slots 5,6,7,21,50,80,89,94.
    ...
    for (std::size_t ordinal = 0U; ordinal < std::size(expected_slots); ++ordinal) {
        ...
        assert(identity.canonical_display_name ==
            "em035_037_00" + std::to_string(ordinal) + ".txt");
    }
    ...
    // Duplicate physical slots cannot receive external naming authority.
