    ...
    for (const auto& entry : parsed.document.entries) {
        ...
        if (entry.populated) {
            ...
            if (!range_valid) {
                ...
                // A synthetic slot name (slot_0002.bin) is generated by the
                // parser, not carried by the container. Feeding it to the
                // classifier would let the generated suffix masquerade as a
                // format observation, so only a real stored name is used as a
                // naming hint. Byte identity still decides either way.
                const auto classification = ResourceClassifier::classify(
                    entry.synthetic_name ? std::string_view{}
                                         : std::string_view{entry.logical_name},
