...
[[nodiscard]] std::string parse_cache_key(
    ...
    if (!provenance.valid()) {
        // Invalid lineage must never be used to merge evidence domains.
...
ContainerTreeExpansion ContainerTreeExpander::expand(
    ...
    // Cache only parser output. Identity expansion always runs for each
    // ResourcePayload, including duplicate/alias slots that share bytes.
