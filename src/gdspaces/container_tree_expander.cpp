...
ContainerTreeExpansion ContainerTreeExpander::expand(
    ...
    // This cache reuses only parser results, never resource identities. A
    // parser must explicitly declare byte-only semantics and the payload must
    // carry valid immutable byte provenance before reuse is allowed.
    ...
    visit = [&](const ResourcePayload& payload, std::size_t depth, bool is_root) {
        ...
        if (parser == nullptr) {
            ...
            add_diagnostic(
                ...
                "No registered container parser recognizes this resource; the resource identity remains in the graph without nested expansion.",
                payload.resource.id);
            return;
        }
