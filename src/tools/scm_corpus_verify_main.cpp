...
using dmc::rengine::formats::ParseDiagnostic;
using dmc::rengine::formats::scm::Parser;
using dmc::rengine::formats::scm::WriteMode;
using dmc::rengine::formats::scm::Writer;
...
[[nodiscard]] bool is_scm_path(std::string_view path) {
    const auto extension = std::filesystem::path{path}.extension().string();
    return lower_copy(extension) == ".scm";
}

[[nodiscard]] std::optional<std::size_t> first_mismatch(
...
[[nodiscard]] std::size_t count_diagnostics(
    std::span<const ParseDiagnostic> diagnostics) noexcept {
    return diagnostics.size();
}

[[nodiscard]] FileResult verify_one(
    ...
    result.diagnostic_count += count_diagnostics(parsed.diagnostics);
    ...
    result.diagnostic_count += count_diagnostics(preserved.diagnostics);
    ...
    result.diagnostic_count += count_diagnostics(rebuilt.diagnostics);
    if (rebuilt.wrote) {
...
void print_usage() {
    std::cerr
        << "usage: scm-corpus-verify <directory> [--json <report.json>]\n";
}
...
int main(int argc, char** argv) {
    ...
    if (argc == 4 && std::string_view{argv[2]} == "--json") {
        json_path = std::filesystem::path{argv[3]};
    } else if (argc != 2) {
        print_usage();
        return 1;
    }
    ...
    if (!std::filesystem::is_directory(root, error) || error) {
        std::cerr << "scm-corpus-verify: not a readable directory: "
                  << root.string() << '\n';
        return 2;
    }
    ...
    if (!registry.mount(std::make_unique<LocalDirectorySource>(
        ...
        std::cerr << "scm-corpus-verify: failed to mount corpus directory\n";
        return 3;
    }
    ...
    resources.erase(
        std::remove_if(
            ...
            [](const ResourceRef& resource) {
                return !is_scm_path(resource.id.logical_path);
            }),
        resources.end());
    std::sort(
        resources.begin(), resources.end(),
        [](const ResourceRef& lhs, const ResourceRef& rhs) {
    ...
    for (const auto& resource : resources) {
        const auto result = verify_one(registry, resource);
        std::cout
            << (result.canonical_bit_identical ? "BIT_IDENTICAL" :
    ...
    const auto json = build_json(root, results);
    if (json_path.has_value()) {
        std::ofstream stream(*json_path, std::ios::binary | std::ios::trunc);
        if (!stream) {
            std::cerr << "scm-corpus-verify: cannot write report: "
                      << json_path->string() << '\n';
            return 4;
        }
        stream << json;
        if (!stream) {
            std::cerr << "scm-corpus-verify: failed while writing report\n";
            return 4;
        }
    } else {
        std::cout << json;
