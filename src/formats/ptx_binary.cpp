...
std::optional<binary::Document> build_binary_document(
    ...
    if (!document.add_region(binary::Region{
        ...
        !add_u32_field(document, reader, "ptx-texture-count", "Texture count", 0U)) {
        return std::nullopt;
    }

    for (const auto& texture : scan.framing.document.textures) {
        ...
        if (!document.add_region(binary::Region{
                ...
                .name = "PTX texture descriptor " + std::to_string(texture.texture_index),
                .range = {.offset = texture.descriptor_offset, .size = descriptor_size},
            ...
            !document.add_region(binary::Region{
                ...
                .range = {.offset = texture.dds_offset, .size = texture.dds_size},
        ...
        const auto descriptor_offset = static_cast<std::size_t>(texture.descriptor_offset);
        if (!add_u32_field(document, reader, prefix + "-encoding", "Encoding", descriptor_offset + 0x08U) ||
            !document.add_field(binary::Field{
                ...
                .display_value = std::to_string(texture.width) + "x" + std::to_string(texture.height),
                .parent_id = {},
                .evidence_id = {},
            }) ||
            !add_u32_field(document, reader, prefix + "-payload-size", "DDS payload size", descriptor_offset + 0x38U) ||
            !add_u32_field(document, reader, prefix + "-aux-mode", "Auxiliary mode", descriptor_offset + 0x3CU) ||
            !add_u32_field(document, reader, prefix + "-aux-value", "Auxiliary value", descriptor_offset + 0x40U) ||
            !document.add_field(binary::Field{
                ...
                .kind = binary::FieldKind::structure,
                .type_name = "packed_u16_width_height",
                .display_value = std::to_string(texture.secondary_width) + "x" + std::to_string(texture.secondary_height),
                .parent_id = {},
                .evidence_id = {},
            }) ||
            !document.add_field(binary::Field{
            ...
            !add_u32_field(document, reader, prefix + "-dds-size", "DDS total size", descriptor_offset + 0x64U)) {
            return std::nullopt;
        }
        ...
        if (record_end > dds_end && !document.add_region(binary::Region{
                .id = prefix + "-padding",
                .name = "Texture alignment padding " + std::to_string(texture.texture_index),
                .range = {.offset = dds_end, .size = record_end - dds_end},
    ...
    static_cast<void>(document.add_ownership(binary::OwnershipClaim{
        ...
        .rationale = "The modular PTX reader owns the validated DMC3 texture-bundle framing.",
    }));
    return document;
}
