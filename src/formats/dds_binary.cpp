...
std::optional<binary::Document> build_binary_document(
    ...
    static_cast<void>(document.add_ownership(binary::OwnershipClaim{
        ...
        .range = {.offset = 0U, .size = header_size},
        .rationale = "The modular DDS reader owns the corpus-confirmed 128-byte DMC3 DDS header.",
    }));
    static_cast<void>(document.add_ownership(binary::OwnershipClaim{
        .owner_id = "formats.dds",
        .range = {
        ...
        .rationale = "The modular DDS reader owns the bounded DXT mip payload extent without decoding texels.",
    }));
    return document;
}
