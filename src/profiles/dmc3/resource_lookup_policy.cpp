...
bool ResourceLookupAttempt::valid() const noexcept {
    ...
    switch (provider) {
    case ResourceProviderClass::archive: return provider_mask == 1U;
    case ResourceProviderClass::physical: return provider_mask == 2U;
    }
    return false;
}
...
ResourceLookupPlan ResourceLookupPolicy::plan(std::string_view logical_path) {
    ...
    for (const auto provider : {
        ...
        for (std::size_t prefix_index = 0U;
             prefix_index < prefixes.size(); ++prefix_index) {
