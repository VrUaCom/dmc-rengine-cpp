...
[[nodiscard]] bool candidate_fits(std::string_view value) noexcept {
    // 0x14002FCA0 passes 0x400 as the destination bound to the path
    // concatenation helper. Reserve one byte for the terminating NUL.
    ...
    for (std::size_t index = 0U; index < attempts.size(); ++index) {
        ...
        if (attempts[index].provider != expected_provider ||
            ...
            attempts[index].candidate !=
                attempts[index].prefix + basename) {
            return false;
        }
    }
    return true;
