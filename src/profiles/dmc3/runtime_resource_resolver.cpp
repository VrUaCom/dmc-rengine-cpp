...
bool RuntimeSourceBindings::valid_for(
    ...
    if (!topology.valid() ||
        topology.physical_root_mounted != !physical_source_id.empty() ||
    ...
    for (std::size_t index = 0U; index < archives.size(); ++index) {
        ...
        if (!binding.valid() ||
            binding.volume_index >= topology.discovery_first_missing_index ||
...
RuntimeResolutionReport RuntimeResourceResolver::resolve(
    ...
    if (!bindings.valid_for(topology)) {
        return invalid_configuration(
            request,
            "Runtime source bindings do not exactly match the explicitly successful mount topology.");
    }
    ...
    if (topology.physical_root_mounted) {
        ...
        if (physical_source == nullptr ||
            ...
            return invalid_configuration(
                request,
                "The physical source binding does not reference the exact successfully mounted source instance.");
        }
        ...
        if (direct_physical_source == nullptr) {
            ...
            if (!physical_index.has_value()) {
                return invalid_configuration(
                    request,
                    "The mounted physical source exposes no direct path capability and its current enumeration cannot produce a clean 0x0C product lookup index.");
            }
        }
    }
    ...
    for (const auto& volume : topology.mounted_archives) {
        ...
        if (binding == nullptr) {
            return invalid_configuration(
                request,
                "A successfully mounted runtime archive volume has no source binding.");
        }
        ...
        if (!index.has_value()) {
            return invalid_configuration(
                request,
                "A mounted archive source is missing or its current enumeration cannot produce a clean 0x0E lookup index.");
        }
    ...
    for (const auto& attempt : plan.attempts) {
            ...
            for (const auto volume_index : topology.archive_resolution_order) {
                ...
                if (binding == nullptr || mounted_index == nullptr ||
                    ...
                    return invalid_configuration(
                        request,
                        std::move(probes),
                        "Archive precedence references a volume without its current successfully mounted source index.");
                }
        ...
        // The recovered resolver traverses linked mount nodes. If physical
        // registration did not successfully link a type-0 node, there is no
        // physical provider operation to report as a synthetic miss.
        if (!topology.physical_root_mounted) {
            continue;
        }
    ...
    std::string detail;
    if (!topology.physical_root_mounted) {
        detail = "All lookup attempts across the explicitly successful archive mount set completed without a resource hit. The successful topology contains no type-0 physical node, so no physical provider miss was synthesized.";
    } else if (direct_physical_source != nullptr) {
        detail = "All canonical archive and mounted-physical lookup attempts completed without a resource hit. Physical probes used the source-native direct path capability and remain product-classified outside the bounded Windows parity receipts.";
    } else {
        detail = "All canonical archive and mounted-physical lookup attempts completed without a resource hit. The mounted physical source exposes no direct path capability, so probes used the explicitly product-classified 0x0C index fallback.";
    }
