...
[[nodiscard]] bool exact_parent_span(
    ...
    if (!child.entry.populated ||
        child.entry.offset > static_cast<std::uint64_t>(parent.bytes.size()) ||
        child.entry.size >
            static_cast<std::uint64_t>(parent.bytes.size()) - child.entry.offset ||
...
[[nodiscard]] bool same_span(
    const gdspaces::ContainerChild& left,
    const gdspaces::ContainerChild& right) noexcept {
    return left.entry.offset == right.entry.offset &&
...
bool NestedReintegrationReceipt::valid() const noexcept {
    ...
    return std::all_of(
        ...
        [](const NestedSpanReceipt& span) { return span.valid(); });
}
