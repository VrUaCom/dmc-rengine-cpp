...
StageResourceTableReadResult StageResourceTableReader::read(
    ...
    for (std::uint32_t row_index = 0U;
        ...
        for (std::uint32_t column_index = 0U;
            ...
            const auto path_pointer = reader.u64_le(
                static_cast<std::size_t>(pointer_field_offset));
            ...
            const auto path_file_offset = image.rva_to_file_offset(
                static_cast<std::uint32_t>(path_rva64));
            ...
            const auto path = read_ascii_c_string(
                executable_bytes,
                *path_file_offset,
                max_path_bytes);
            if (!path.has_value()) {
