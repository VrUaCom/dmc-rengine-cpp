...
// floating-point ones, so a mobile libc++ (Android NDK, iOS) can define
// __cpp_lib_to_chars while still lacking from_chars(double). The fallback
// deliberately uses a classic-locale stream rather than strtod: strtod
// honours the global C locale, and one whose decimal separator is not '.'
