...
void analyze_scm(
    ...
    // Deep-reader promotion: a valid SCM exposes both the canonical physical
    // byte map and evidence-backed runtime provenance to the shared Binary
    // Inspector. Preservation-only fields remain explicit; rejected candidate
    // consumers are recorded as negative evidence rather than invented names.
    if (parsed.ok()) {
        static_cast<void>(native_reader_support::attach_binary_document(
            project,
            report,
            session.resource().id,
            formats::scm::build_deep_binary_document(
