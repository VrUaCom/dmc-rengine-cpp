...
#else
  (void)instance; (void)native_window; (void)surface;
  return false;
#endif
}
...
bool create_render_targets(VulkanRenderer::Impl& state) {
  ...
  attachment.format = state.format; attachment.samples = VK_SAMPLE_COUNT_1_BIT;
  ...
  for (std::size_t i = 0; i < state.views.size(); ++i) {
    ...
    framebuffer.renderPass = state.render_pass; framebuffer.attachmentCount = 1; framebuffer.pAttachments = &state.views[i];
    framebuffer.width = state.extent.width; framebuffer.height = state.extent.height; framebuffer.layers = 1;
...
std::vector<std::uint32_t> load_spirv(void* asset_manager, const char* name) {
  ...
  if (length <= 0 || (length % 4) != 0) { AAsset_close(asset); return {}; }
  ...
  return words;
#elif defined(_WIN32) && defined(RENGINE_LSG_SHADER_DIR)
  (void)asset_manager;
  const auto path = std::filesystem::path{RENGINE_LSG_SHADER_DIR} / name;
  std::ifstream stream(path, std::ios::binary | std::ios::ate);
  if (!stream) return {};
  const auto length = stream.tellg();
  if (length <= 0 || (static_cast<std::uint64_t>(length) % 4ull) != 0ull) return {};
  std::vector<std::uint32_t> words(static_cast<std::size_t>(length) / sizeof(std::uint32_t));
  stream.seekg(0, std::ios::beg);
  stream.read(reinterpret_cast<char*>(words.data()), length);
  return stream ? words : std::vector<std::uint32_t>{};
#else
  (void)asset_manager; (void)name;
  return {};
#endif
}
...
bool create_pipeline(VulkanRenderer::Impl& state, void* asset_manager) {
  ...
  stages[0] = {VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO}; stages[0].stage = VK_SHADER_STAGE_VERTEX_BIT; stages[0].module = vertex_module; stages[0].pName = "main";
  stages[1] = {VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO}; stages[1].stage = VK_SHADER_STAGE_FRAGMENT_BIT; stages[1].module = fragment_module; stages[1].pName = "main";
  ...
  VkPushConstantRange push_range{}; push_range.stageFlags = VK_SHADER_STAGE_FRAGMENT_BIT; push_range.size = sizeof(PushConstants);
  VkPipelineLayoutCreateInfo layout{VK_STRUCTURE_TYPE_PIPELINE_LAYOUT_CREATE_INFO}; layout.pushConstantRangeCount = 1; layout.pPushConstantRanges = &push_range;
  ...
  pipeline.stageCount = 2; pipeline.pStages = stages; pipeline.pVertexInputState = &vertex_input; pipeline.pInputAssemblyState = &assembly;
  pipeline.pViewportState = &viewport_state; pipeline.pRasterizationState = &raster; pipeline.pMultisampleState = &multisample;
  pipeline.pColorBlendState = &blend; pipeline.layout = state.pipeline_layout; pipeline.renderPass = state.render_pass;
...
bool VulkanRenderer::initialize(void* native_window, void* asset_manager) {
  ...
  const char* platform_extension = platform_surface_extension();
  if (platform_extension == nullptr) return false;
  const char* instance_extensions[] = {VK_KHR_SURFACE_EXTENSION_NAME, platform_extension};
  VkApplicationInfo app{VK_STRUCTURE_TYPE_APPLICATION_INFO}; app.pApplicationName = "Rengine LSG Prototype";
  app.applicationVersion = VK_MAKE_VERSION(0, 2, 0); app.pEngineName = "DMC Rengine"; app.engineVersion = VK_MAKE_VERSION(0, 2, 0); app.apiVersion = VK_API_VERSION_1_2;
  VkInstanceCreateInfo instance{VK_STRUCTURE_TYPE_INSTANCE_CREATE_INFO}; instance.pApplicationInfo = &app;
  instance.enabledExtensionCount = static_cast<std::uint32_t>(std::size(instance_extensions)); instance.ppEnabledExtensionNames = instance_extensions;
  ...
  if (!choose_device(state) || !create_device(state) || !create_swapchain(state, native_window) || !create_render_targets(state) ||
...
bool VulkanRenderer::draw_frame(float time_seconds, std::uint32_t character_index, bool detail_enabled) noexcept {
  ...
  VkCommandBufferBeginInfo begin{VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO}; if (vkBeginCommandBuffer(command, &begin) != VK_SUCCESS) return false;
  VkClearValue clear{}; clear.color.float32[0] = 0.025f; clear.color.float32[1] = 0.035f; clear.color.float32[2] = 0.055f; clear.color.float32[3] = 1.0f;
  VkRenderPassBeginInfo render{VK_STRUCTURE_TYPE_RENDER_PASS_BEGIN_INFO}; render.renderPass = state.render_pass; render.framebuffer = state.framebuffers[image_index];
  render.renderArea.extent = state.extent; render.clearValueCount = 1; render.pClearValues = &clear;
  vkCmdBeginRenderPass(command, &render, VK_SUBPASS_CONTENTS_INLINE); vkCmdBindPipeline(command, VK_PIPELINE_BIND_POINT_GRAPHICS, state.pipeline);
  const PushConstants push{character_index & 1u, detail_enabled ? 1u : 0u, time_seconds, 0.0f};
  vkCmdPushConstants(command, state.pipeline_layout, VK_SHADER_STAGE_FRAGMENT_BIT, 0, sizeof(push), &push); vkCmdDraw(command, 3, 1, 0, 0); vkCmdEndRenderPass(command);
  if (vkEndCommandBuffer(command) != VK_SUCCESS) return false;
  const VkPipelineStageFlags wait_stage = VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT;
  VkSubmitInfo submit{VK_STRUCTURE_TYPE_SUBMIT_INFO}; submit.waitSemaphoreCount = 1; submit.pWaitSemaphores = &state.image_available; submit.pWaitDstStageMask = &wait_stage;
  submit.commandBufferCount = 1; submit.pCommandBuffers = &command; submit.signalSemaphoreCount = 1; submit.pSignalSemaphores = &state.render_finished;
  if (vkQueueSubmit(state.queue, 1, &submit, state.in_flight) != VK_SUCCESS) return false;
  VkPresentInfoKHR present{VK_STRUCTURE_TYPE_PRESENT_INFO_KHR}; present.waitSemaphoreCount = 1; present.pWaitSemaphores = &state.render_finished;
  present.swapchainCount = 1; present.pSwapchains = &state.swapchain; present.pImageIndices = &image_index;
  const auto result = vkQueuePresentKHR(state.queue, &present); return result == VK_SUCCESS || result == VK_SUBOPTIMAL_KHR;
}
